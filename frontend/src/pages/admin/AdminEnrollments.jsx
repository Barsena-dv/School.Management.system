        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="bg-surface w-full max-w-md rounded-lg shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-5 border-b border-border">
                    <div>
                        <h3 className="text-lg font-bold text-text-primary">Create Enrollment</h3>
                        <p className="text-xs text-text-muted font-medium mt-0.5">Enroll a student in a subject.</p>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <div className="bg-surface w-full max-w-sm rounded-lg shadow-2xl p-6 space-y-5">
                <div className="flex flex-col items-center text-center">
                    ...
                    <h3 className="text-lg font-bold text-text-primary">Remove Enrollment?</h3>
    ...
    return (
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-border">
                <div>
                    <h1 className="text-xl font-bold font-heading text-text-primary tracking-tight">Enrollments</h1>
                    <p className="text-text-muted mt-1 font-medium">Manage student-subject enrollment records.</p>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        <div className="w-16 h-16 bg-bg-subtle rounded-lg flex items-center justify-center mb-4 border border-border">
                            <Link2 className="w-7 h-7 text-text-muted/40" />
                        </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-border">
                                {filtered.map(e => (
                                    <tr key={e._id} className="hover:bg-bg-subtle/30 transition-colors group">

                                        {/* Student */}
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-lg bg-primary-subtle border border-primary/20 flex items-center justify-center flex-shrink-0 text-xs font-bold text-primary group-hover:bg-blue-100 transition-colors">
                                                    {e.student?.user?.name?.charAt(0)?.toUpperCase() || '?'}
                                                </div>
                                                <div>
                                                    <p className="text-sm font-bold text-text-primary">{e.student?.user?.name || '—'}</p>
                                                    <p className="text-[11px] text-text-muted font-medium">Roll: {e.student?.rollNumber || '—'}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-5 py-4">
                                            {e.subject?.teacher ? (
                                                <div className="flex items-center gap-2">
                                                    <div className="w-6 h-6 rounded-lg bg-bg-subtle flex items-center justify-center text-[10px] font-bold text-text-secondary">
                                                        {e.subject.teacher.name?.charAt(0)?.toUpperCase()}
                                                    </div>
                                                    <span className="text-sm text-text-secondary font-medium">{e.subject.teacher.name}</span>
                                                </div>
                                            ) : <span className="text-xs text-text-muted/40">—</span>}
                                        </td>
